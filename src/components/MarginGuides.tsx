...
export const MarginGuides = ({
  ...
  const handleSpinner = (margin: 'top' | 'right' | 'bottom' | 'left', direction: 'up' | 'down') => {
    const currentValue = (() => {
      switch (margin) {
        ...
        case 'right':
          return parseFloat(marginRight) || 0;
        case 'bottom':
  ...
  const SpinnerInput = ({
    ...
    position: 'top' | 'right' | 'bottom' | 'left';
      ...
      <input type="text" value={formatValue(value)} onChange={e => onChange(e.target.value)} onClick={onClick} className={`w-20 px-2 py-1 text-sm rounded-lg text-center border focus:outline-none focus:ring-2 focus:ring-blue-500 focus:border-transparent transition-all duration-200 ${isSelected ? 'bg-blue-500 text-white border-blue-400' : 'bg-slate-600 text-slate-200 border-slate-500 hover:bg-slate-500'}`} readOnly />
  ...
  return <div className="bg-slate-800/30 rounded-lg p-6 space-y-6 py-0 px-[10px]">
      <div className="flex items-center justify-between">
        <h4 className="text-lg font-semibold text-slate-200">Margin Guides</h4>
        <div className="flex items-center gap-2">
          <Popover open={showColorPicker} onOpenChange={setShowColorPicker}>
            <PopoverTrigger asChild>
              <button className="w-8 h-8 rounded-lg border-2 border-slate-600/60 hover:border-slate-500/80 transition-colors" style={{
              backgroundColor: guideColor
            }} />
            </PopoverTrigger>
            <PopoverContent className="w-auto p-0" align="end">
              <ColorPicker selectedColor={guideColor} onColorSelect={color => {
              setGuideColor(color);
          ...
          <button onClick={() => setProportionLocked(!proportionLocked)} className={`w-8 h-8 rounded-lg border-2 transition-colors flex items-center justify-center ${proportionLocked ? 'bg-blue-500 border-blue-400 text-white' : 'border-slate-600/60 hover:border-slate-500/80 text-slate-300'}`}>
      ...
      <div className="relative bg-slate-700/40 rounded-xl p-12 flex items-center justify-center">
        ...
        <div className="absolute top-4 left-1/2 transform -translate-x-1/2">
          <SpinnerInput value={marginTop} onChange={value => handleInputChange('top', value)} onClick={() => handleMarginClick('top')} onSpinner={direction => handleSpinner('top', direction)} isSelected={selectedMargin === 'top'} position="top" />
        </div>
        
        {/* Left Margin Input */}
        <div className="absolute left-4 top-1/2 transform -translate-y-1/2">
          <SpinnerInput value={marginLeft} onChange={value => handleInputChange('left', value)} onClick={() => handleMarginClick('left')} onSpinner={direction => handleSpinner('left', direction)} isSelected={selectedMargin === 'left'} position="left" />
        </div>
        
        {/* Right Margin Input */}
        <div className="absolute right-4 top-1/2 transform -translate-y-1/2">
          <SpinnerInput value={marginRight} onChange={value => handleInputChange('right', value)} onClick={() => handleMarginClick('right')} onSpinner={direction => handleSpinner('right', direction)} isSelected={selectedMargin === 'right'} position="right" />
        </div>
        
        {/* Bottom Margin Input */}
        <div className="absolute bottom-4 left-1/2 transform -translate-x-1/2">
          <SpinnerInput value={marginBottom} onChange={value => handleInputChange('bottom', value)} onClick={() => handleMarginClick('bottom')} onSpinner={direction => handleSpinner('bottom', direction)} isSelected={selectedMargin === 'bottom'} position="bottom" />
        </div>

        {/* Central Square Canvas Area with gradient borders */}
        <div className={`w-32 h-32 rounded-2xl bg-slate-600/20 transition-all duration-300 ${getBorderStyle()}`} style={getBorderGradientStyle()}>
        </div>
      </div>

      {/* Controls Row */}
      <div className="flex items-center justify-between">
        <select value={currentPreset} onChange={e => handlePresetChange(e.target.value as keyof typeof PRESETS)} className="bg-slate-700 text-slate-200 border border-slate-600 rounded-lg px-3 py-2 text-sm focus:outline-none focus:ring-2 focus:ring-blue-500 flex-1 mr-3">
        ...
        <button onClick={handleReset} className="px-4 py-2 bg-slate-700 hover:bg-slate-600 text-slate-200 rounded-lg border border-slate-600 text-sm transition-colors">
          Reset
        </button>
      </div>
